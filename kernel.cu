...
// DEVICE COPIES OF LOOKUP TABLES
...
__device__ void AES_encrypt(const uint *pt, uint *ct, const uint *rek, uint Nr) {
...
__device__ void print_hex(const uint8_t *data, uint32_t length) {
...
#define FLIP32(x) (((x) >> 24) | (((x) >> 8) & 0xff00) | (((x) << 8) & 0xff0000) | (((x) << 24) & 0xff000000))

#define FLIP64(x)	 (((x) << 56) |          \
                   (((x) & 0xff00) << 40) | \
                   (((x) & 0xff0000) << 24) | \
                   (((x) & 0xff000000) << 8)  | \
                   (((x) & 0xff00000000) >> 8)  | \
...
__global__ void test_AES(const uint32_t *key, uint8_t *text) {
  print_hex(text, 16);
  uint32_t state[4];
...
#pragma unroll
  ...
  AES_encrypt(state, state, key, 14);
#pragma unroll
  for (int i = 0; i < 4; i++) {
    state[i] = FLIP32(state[i]);
  }
...
__global__ void go(uint32_t base, const uint32_t * __restrict__ akey, const uint32_t * __restrict bkey, uint64_t ai0, uint64_t ai1, uint64_t bi0, uint64_t bi1, uint32_t *success, uint32_t difficulty /* , uint8_t *dists */) {
  ...
  */
  ...
  /* doAes((uint8_t *) aj, akey); */
  /* doAes((uint8_t *) bj, bkey); */

  uint32_t *aj_32 = (uint32_t *) aj;
  uint32_t *bj_32 = (uint32_t *) bj;

#pragma unroll
  for (int i = 0; i < 4; i++) {
    aj_32[i] = FLIP32(aj_32[i]);
    bj_32[i] = FLIP32(bj_32[i]);
  }

  AES_encrypt((const uint32_t *) aj, (uint32_t *) aj, akey, 14);
  AES_encrypt((const uint32_t *) bj, (uint32_t *) bj, bkey, 14);
...
#pragma unroll
  ...
  uint8_t *aj_byte = (uint8_t *) aj;
  uint8_t *bj_byte = (uint8_t *) bj;
  uint8_t tmp;
...
#pragma unroll
  for (uint8_t i = 0; i < 8; i++) {
    tmp = aj_byte[i];
    aj_byte[i] = aj_byte[15-i];
    aj_byte[15-i] = tmp;
    tmp = bj_byte[i];
    bj_byte[i] = bj_byte[15-i];
    bj_byte[15-i] = tmp;
  }
