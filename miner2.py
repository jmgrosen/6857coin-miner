...
__device__ __forceinline__ static void cn_aes_pseudo_round_mut(const uint32_t * __restrict__ sharedMemory, uint32_t * __restrict__ val, const uint32_t * __restrict__ expandedKey)
{
	...
	round(sharedMemory, b1, val, expandedKey + 10 * N_COLS);
	round(sharedMemory, val, b1, expandedKey + 11 * N_COLS);
	round(sharedMemory, b1, val, expandedKey + 12 * N_COLS);
	round(sharedMemory, val, b1, expandedKey + 13 * N_COLS);
}
