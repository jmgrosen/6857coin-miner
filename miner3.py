...
ffi = FFI()
ffi.cdef("int go(uint64_t, uint64_t, const uint8_t*, const uint8_t*, const uint8_t*, const uint8_t*, uint32_t, uint64_t*);")
...
def solve_block(b):
    ...
    akey_buf = ffi.new("char[]", seed)
    bkey_buf = ffi.new("char[]", seed2)
    ai, _, bi, _ = compute_ciphers(b)
    ai_buf = ffi.new("char[]", ai)
    bi_buf = ffi.new("char[]", bi)
    ...
    while True:
        ...
        if lib.go(base, SIZE, akey_buf, bkey_buf, ai_buf, bi_buf, d, out):
            break
